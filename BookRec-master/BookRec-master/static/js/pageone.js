...
var asin = []
  ...
  $.ajax({
    ...
    success: function(result){
      ...
      $.each(result,function(index,element){
          ...
          switch(element.Title){
            ...
            $('#rowwrapper').append('<div class="col-lg-3 col-md-4 col-sm-6 portfolio-item">\
      <div class="card h-70">\
            <img class="cover"  height="300" width ="250" src="'+cover+'" >\
            <div class="card-body">\
              <h4 class="card-title">\
                <p>'+element.Title+'</p>\
              </h4>\
   ...
   console.log("Rating:"+this.id+data.rating)
	...
	$('#rowwrapper').after('<a style ="color: #ffffff" id="btnRec" href="#vis" class="button button-3d button-primary button-rounded">Show my recommendations!</a>')
...
function show_heatmap(data,name){
  ...
  $('#heatmap').html('')
  $('#heatmap').append('<h4>When do people read '+name+'</h4>')
...
function add_title(title,author){
  ...
  $("#titletext").html("");
  $('#vis').after('<h3 id = "titletext">Here are some details about '+title+' by '+author+' </h3>');
